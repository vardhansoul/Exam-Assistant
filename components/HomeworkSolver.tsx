...
const STORAGE_KEY = 'homework_solver_state';
...
const HomeworkSolver: React.FC<HomeworkSolverProps> = ({ language, isOnline, user, canAccessPremium, requestAuth }) => {
    ...
    const handleSolve = async () => {
        ...
        logActivity(user?.uid || null, {
            ...
            view: AppView.HOMEWORK_SOLVER,
            context: {}
        });

        try {
            const { base64, mimeType } = await fileToGenerativePart(image.file);
            const userPrompt = prompt || 'Solve the homework problem in this image.';
            const result = await solveHomeworkProblem(base64, mimeType, userPrompt, language);
            setResponse(result);
        } catch (err) {
        ...
        <div className="max-w-4xl mx-auto">
            <Card>
                <div className="text-center">
                    <h2 className="text-xl sm:text-2xl font-bold text-slate-800">COC AI Homework Solver</h2>
                    <p className="text-slate-500 mt-2">Get step-by-step solutions for your homework problems.</p>
                </div>

                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    {/* Image Upload and Preview */}
                    <div className="space-y-4">
                        <h3 className="font-bold text-slate-700">1. Upload your homework</h3>
                        {image ? (
                             <div className="relative group">
                                <img src={image.preview} alt="Homework preview" className="w-full h-auto rounded-lg border-2 border-slate-300" />
                        ...
                        <h3 className="font-bold text-slate-700 pt-4">2. Add context (optional)</h3>
                         <textarea
                            ...
                            placeholder="e.g., Solve for x, or show the work for question 3."
                            ...
                            rows={3}
                            disabled={!image && !response}
                         />
                         <Button onClick={handleSolve} disabled={!image || isLoading || !isOnline} className="w-full !py-3">
                    ...
                    {/* Response Area */}
                     <div className="space-y-4">
                        <h3 className="font-bold text-slate-700">3. View the solution</h3>
                        <div className="p-4 border border-slate-200 rounded-lg bg-slate-50 min-h-[300px] max-h-[70vh] overflow-y-auto">
                            {isLoading && (
                                <div className="flex flex-col justify-center items-center h-full text-center">
                                    <LoadingSpinner />
                                    <p className="mt-4 text-slate-600">COC AI is working on the solution...</p>
                                </div>
                            )}
                            <ErrorMessage message={error} onRetry={handleSolve} />
                            {response && (
                                <ContentRenderer content={response} className="prose prose-slate max-w-none" />
                            )}
                            {!isLoading && !response && !error && (
                                <div className="flex justify-center items-center h-full">
                                    <p className="text-slate-500 text-center">Your step-by-step solution will appear here.</p>
                                </div>
                            )}
                        </div>
                    </div>
