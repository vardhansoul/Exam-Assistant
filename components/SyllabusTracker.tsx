      ...
      <div className="flex items-center group">
        {hasChildren && (
          <button onClick={() => setIsExpanded(!isExpanded)} className="p-1 rounded-full hover:bg-slate-200 mr-1">
             <ChevronDownIcon className={`w-4 h-4 text-slate-500 transition-transform duration-200 ${!isExpanded && '-rotate-90'}`} />
          </button>
        )}
    ...
    <div className="max-w-3xl mx-auto">
        <Card>
          <div className="text-center">
            ...
            <p className="text-slate-500 mt-2">Generate an interactive syllabus and track your progress.</p>
          </div>
          
          <div className="mt-6">
            <Button onClick={handleGenerateSyllabus} disabled={isLoading || !isOnline} className="w-full !py-3">
          ...
          {!isLoading && syllabus && (
            <div className="mt-6">
              <div className="mb-4">
                  ...
                  <span className="text-sm font-medium text-indigo-700">{checkedIds.size} / {totalTopics} topics ({Math.round(progressPercentage)}%)</span>
                  </div>
                  <div className="w-full bg-slate-200 rounded-full h-4">
                  <div className="bg-indigo-600 h-4 rounded-full transition-all duration-500" style={{ width: `${progressPercentage}%` }}></div>
                  </div>
              </div>
