...
const StudyMaterialModal: React.FC<StudyMaterialModalProps> = ({ isOpen, onClose, topic, material, isLoading, error, onRetry, language, selectionPath, isOnline, user, onSelectRelatedTopic }) => {
  ...
  useEffect(() => {
    ...
    // user check ensures we are either logged in or in valid trial (since parent handles premium check)
    // Actually user might be null if trial. So check if material exists (means we are allowed).
    if (material && topic && isOnline) {
      const fetchDeepDive = async () => {
        setIsDeepDiveLoading(true);
        try {
          // Pass !user as isTrial
          const deepDiveData = await generateDeepDiveForTopic(topic, language, selectionPath, !user);
  ...
  return (
    <div className="fixed inset-0 bg-slate-100 dark:bg-slate-900 z-50 flex flex-col animate-slide-up print-container">
      <div className="w-full h-full flex flex-col">
          ...
          <div className="max-w-5xl mx-auto h-full">
            {isLoading && (
                <div className="flex flex-col justify-center items-center h-full text-center no-print">
                    <LoadingSpinner />
                    <p className="mt-4 text-slate-600 dark:text-slate-400 text-lg">COC AI is preparing your study material...</p>
                </div>
            )}
        ...
        <footer className="p-4 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-end items-center flex-shrink-0 no-print">
